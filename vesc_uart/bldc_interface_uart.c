...
// Private functions
static void process_packet(unsigned int uart_index, unsigned char *data, unsigned int len);
static void send_packet_bldc_interface(unsigned int uart_index, unsigned char *data, unsigned int len);
 ...
 */
void bldc_interface_uart_init(unsigned int uart_index, void(*func)(unsigned int uart_index, unsigned char *data, unsigned int len)) {
	// Initialize packet handler
	packet_init(func, process_packet, uart_index);
 ...
 */
void bldc_interface_uart_process_byte(unsigned int uart_index, unsigned char b) {
	packet_process_byte(b, uart_index);
}
...
static void process_packet(unsigned int uart_index, unsigned char *data, unsigned int len) {
	...
	bldc_interface_process_packet(uart_index, data, len);
}
...
static void send_packet_bldc_interface(unsigned int uart_index, unsigned char *data, unsigned int len) {
	...
	packet_send_packet(data, len, uart_index);
}
